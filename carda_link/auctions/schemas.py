...
class LotCreateSchema(BaseModel):
    ...
    base_price_per_kg: Decimal = Field(
        ...,
        description="Base starting price per kg in ₹",
    )
...
class LotOutSchema(BaseModel):
    ...
    highest_bid_per_kg: Decimal | None = None
    ...
    def from_orm_model(lot):
        estate_name = (
            lot.harvest_batch.estate.name
            if lot.harvest_batch and lot.harvest_batch.estate
            else "Unknown Estate"
        )
        grade = lot.harvest_batch.grade if lot.harvest_batch else "UNGRADED"
        weight_kg = (
            lot.harvest_batch.weight_kg if lot.harvest_batch else Decimal("0.00")
        )
        return LotOutSchema(
            id=lot.id,
