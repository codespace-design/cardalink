...
class TestAdminAuthentication(TestCase):
    ...
    def test_non_admin_cannot_access_admin_dashboard(self):
        # Log in as seller
        self.client.login(email="seller_test@example.com", password="SellerPassword@123")
...
class TestRegistrationDetails(TestCase):
    ...
    def test_admin_can_view_seller_details(self):
        self.client.login(email="admin_test@example.com", password="AdminPassword@123")
        response = self.client.get(reverse("admin_user_detail", kwargs={"pk": self.seller.pk}))
        self.assertEqual(response.status_code, 200)
        content = response.content.decode()
    ...
    def test_admin_can_view_buyer_details(self):
        self.client.login(email="admin_test@example.com", password="AdminPassword@123")
        response = self.client.get(reverse("admin_user_detail", kwargs={"pk": self.buyer.pk}))
        self.assertEqual(response.status_code, 200)
        content = response.content.decode()
...
class TestApprovalRejectionSuspensionReactivation(TestCase):
    ...
    def test_seller_approval_works(self):
        self.client.login(email="admin_test@example.com", password="AdminPassword@123")
        response = self.client.get(reverse("admin_user_approve", kwargs={"pk": self.seller.pk}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_buyer_approval_works(self):
        self.client.login(email="admin_test@example.com", password="AdminPassword@123")
        response = self.client.get(reverse("admin_user_approve", kwargs={"pk": self.buyer.pk}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_seller_rejection_works(self):
        self.client.login(email="admin_test@example.com", password="AdminPassword@123")
        response = self.client.get(reverse("admin_user_reject", kwargs={"pk": self.seller.pk}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_buyer_rejection_works(self):
        self.client.login(email="admin_test@example.com", password="AdminPassword@123")
        response = self.client.get(reverse("admin_user_reject", kwargs={"pk": self.buyer.pk}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_account_suspension_works(self):
        ...
        self.client.login(email="admin_test@example.com", password="AdminPassword@123")
        response = self.client.get(reverse("admin_user_suspend", kwargs={"pk": self.seller.pk}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_account_reactivation_works(self):
        ...
        self.client.login(email="admin_test@example.com", password="AdminPassword@123")
        response = self.client.get(reverse("admin_user_reactivate", kwargs={"pk": self.seller.pk}))
        self.assertEqual(response.status_code, 302)
...
class TestLoginAndAccessRestrictions(TestCase):
    ...
    def test_active_user_can_login(self):
        logged_in = self.client.login(email="active@example.com", password="Password@123")
        self.assertTrue(logged_in)

    @check_requirement("Login Restrictions")
    def test_pending_user_cannot_login(self):
        logged_in = self.client.login(email="pending@example.com", password="Password@123")
        self.assertFalse(logged_in)

    @check_requirement("Login Restrictions")
    def test_rejected_user_cannot_login(self):
        logged_in = self.client.login(email="rejected@example.com", password="Password@123")
        self.assertFalse(logged_in)

    @check_requirement("Login Restrictions")
    def test_suspended_user_cannot_login(self):
        logged_in = self.client.login(email="suspended@example.com", password="Password@123")
        self.assertFalse(logged_in)
...
class TestSystemHealth(TestCase):
    ...
    def test_migration_check(self):
        ...
        try:
            ...
            if str(e) == "1":
                self.fail(f"Model changes requiring migrations detected: {out.getvalue()} {err.getvalue()}")
