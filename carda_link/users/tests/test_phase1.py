...
class TestUserModel(TestCase):
    ...
    def test_user_model_has_required_fields(self):
        ...
        for name, field in required_fields.items():
            self.assertIn(
                field,
                fields,
                f"User model is missing required field: {name}",
            )
    ...
    def test_email_is_unique(self):
        ...
        self.assertTrue(
            email_field.unique,
            "Email is not configured as unique in the User model.",
        )
...
class TestRolesAndStatuses(TestCase):
    ...
    def test_roles_exist(self):
        self.assertTrue(
            hasattr(User, "Role"),
            "User model does not have a Role inner class/choices.",
        )
        ...
        for r in roles:
            self.assertTrue(
                hasattr(User.Role, r),
                f"{r} role is missing from the User.Role choices.",
            )
        ...
        for r in roles:
            self.assertIn(
                r,
                choices,
                f"{r} role is missing from the User model's role field choices.",
            )
    ...
    def test_statuses_exist(self):
        self.assertTrue(
            hasattr(User, "Status"),
            "User model does not have a Status inner class/choices.",
        )

        statuses = ["PENDING", "ACTIVE", "REJECTED", "SUSPENDED"]
        for s in statuses:
            self.assertTrue(
                hasattr(User.Status, s),
                f"{s} status is missing from the User.Status choices.",
            )
        ...
        for s in statuses:
            self.assertIn(
                s,
                choices,
                f"{s} status is missing from the User model's status field choices.",
            )
...
class TestPasswordSecurity(TestCase):
    ...
    def test_password_is_securely_hashed(self):
        ...
        self.assertTrue(
            user.check_password("PlainPassword"),
            "check_password failed for secure password.",
        )
...
class TestSellerSignup(TestCase):
    ...
    def test_seller_signup_page_works(self):
        ...
        self.assertEqual(
            ...
            200,
            "Seller signup page returned non-200 status code.",
        )
    ...
    def test_seller_fields_exist(self):
        ...
        for f in required_fields:
            if f == "confirm_password":
                self.assertTrue(
                    'name="confirm_password"' in content
                    or 'name="confirm-password"' in content,
                    "Seller signup page is missing confirm_password field.",
                )
            else:
                self.assertTrue(
                    f'name="{f}"' in content
                    or (f == "phone" and 'name="phone_number"' in content),
    ...
    def test_seller_registration_works(self):
        ...
        self.assertIn(
            response.status_code,
            [200, 302],
            "Seller registration post failed.",
        )
        ...
        self.assertEqual(
            user.role,
            User.Role.SELLER,
            "Seller registration did not assign SELLER role.",
        )
        self.assertEqual(
            user.status,
            User.Status.PENDING,
            "Seller registration did not assign PENDING status.",
        )
...
class TestBuyerSignup(TestCase):
    ...
    def test_buyer_signup_page_works(self):
        ...
        self.assertEqual(
            response.status_code,
            200,
            "Buyer signup page returned non-200 status code.",
        )
    ...
    def test_buyer_registration_works(self):
        ...
        self.assertIn(
            response.status_code,
            [200, 302],
            "Buyer registration post failed.",
        )
        ...
        self.assertEqual(
            user.role,
            User.Role.BUYER,
            "Buyer registration did not assign BUYER role.",
        )
        self.assertEqual(
            user.status,
            User.Status.PENDING,
            "Buyer registration did not assign PENDING status.",
        )
...
class TestSignupValidation(TestCase):
    ...
    def test_empty_required_fields_rejected(self):
        ...
        self.assertEqual(
            User.objects.filter(phone_number="9999999999").count(),
            0,
    ...
    def test_invalid_email_rejected(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(
            User.objects.filter(email="not-an-email").exists(),
    ...
    def test_password_mismatch_rejected(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(
            User.objects.filter(email="pwd_mismatch@example.com").exists(),
            "User created with password mismatch.",
        )
    ...
    def test_invalid_farm_area_rejected(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(
            User.objects.filter(email="farm_area_err@example.com").exists(),
            "User created with invalid farm area.",
        )
        ...
        self.assertFalse(
            User.objects.filter(email="farm_area_err@example.com").exists(),
            "User created with negative farm area.",
        )
    ...
    def test_negative_plant_count_rejected(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(
            User.objects.filter(email="plant_err@example.com").exists(),
            "User created with negative cardamom plants.",
        )
    ...
    def test_invalid_area_unit_rejected(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(
            User.objects.filter(email="unit_err@example.com").exists(),
            "User created with invalid area unit.",
        )
    ...
    def test_admin_signup_is_restricted(self):
        ...
        for url in [seller_url, buyer_url]:
            ...
            self.assertNotIn(
                'name="role"',
                res.content.decode(),
                "Form should not expose 'role' field.",
            )
...
class TestDatabaseMigrations(TestCase):
    ...
    def test_no_missing_migrations(self):
        ...
        try:
            ...
            if str(e) == "1":
                self.fail(
                    f"There are model changes requiring migrations. Run 'makemigrations'. Out: {out.getvalue()} Err: {err.getvalue()}",
                )
            else:
                self.fail(
                    f"makemigrations check exited with system exit: {e}. Out: {out.getvalue()} Err: {err.getvalue()}",
                )
